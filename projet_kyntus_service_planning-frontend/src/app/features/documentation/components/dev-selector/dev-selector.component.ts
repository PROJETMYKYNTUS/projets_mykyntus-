...
export class DevSelectorComponent implements OnInit, OnDestroy {
  ...
  /** Listes issues des endpoints filtrÃ©s (aucune donnÃ©e mÃ©tier codÃ©e en dur). */
  ...
  /** Message si lâ€™API org ne rÃ©pond pas ou renvoie une liste vide (aide au diagnostic). */
  ...
  currentTenantId(): string {
    return this.identity.getTenantId() || 'â€”';
  }
  ...
  onRoleChange(role: DocumentationRole): void {
    ...
    // RH / Admin / Audit voient tout le tenant cÃ´tÃ© API : ne pas attendre pÃ´leâ€“celluleâ€“dÃ©partement
    // (sinon scopedUsersForRole reste vide â†’ en-tÃªtes inchangÃ©es â†’ file RH filtrÃ©e comme pilote).
  ...
  displayLine(): string {
    ...
    let scope = '';
    if (sc) {
      scope = ` Â· pÃ©rimÃ¨tre coach ${sc.slice(0, 8)}â€¦`;
    }
    if (sm) {
      scope += ` Â· manager ${sm.slice(0, 8)}â€¦`;
    }
    return `${name} (${r})${scope}`;
  }
  ...
  private startOrganisationCascade(): void {
    ...
    this.data.getOrganisationPoles().subscribe({
      next: (poles) => {
        ...
        if (!poles.length) {
          this.orgLoadHint =
            `Aucun pÃ´le pour le tenant Â« ${this.identity.getTenantId() || 'â€”'} Â». VÃ©rifiez la table documentation.organisation_units (seed SQL 008) et que lâ€™API utilise la mÃªme base.`;
          return;
        }
      ...
      error: (err) => {
        ...
        const hint404 =
          status.includes('404')
            ? ' Les routes /api/documentation/data/organisation/* sont absentes : recompilez et redÃ©marrez le projet DocumentationBackend depuis ce dÃ©pÃ´t. '
            : ' ';
        this.orgLoadHint = `Ã‰chec du chargement des pÃ´les${status}.${hint404}VÃ©rifiez ${environment.apiBaseUrl}, les en-tÃªtes X-Tenant-Id / X-User-Id, et que ce nâ€™est pas un autre service sur ce port.`;
      },
    });
  }
  ...
  private loadCellulesForPole(poleId: string): void {
    this.data.getCellulesByPole(poleId).subscribe({
      next: (cells) => {
        ...
        if (!cells.length) {
          this.departements = [];
          this.orgLoadHint = 'Aucune cellule pour ce pÃ´le (parent_id en base).';
          return;
        }
      ...
      error: () => {
        this.cellules = [];
        this.orgLoadHint = 'Impossible de charger les cellules pour le pÃ´le sÃ©lectionnÃ©.';
      },
    });
  }
  ...
  private loadDepartementsForCellule(celluleId: string): void {
    this.data.getDepartementsByCellule(celluleId).subscribe({
      next: (depts) => {
        ...
        if (!depts.length) {
          this.orgLoadHint = 'Aucun dÃ©partement pour cette cellule (parent_id en base).';
          return;
        }
        this.orgLoadHint = null;
      ...
      error: () => {
        this.departements = [];
        this.orgLoadHint = 'Impossible de charger les dÃ©partements pour la cellule sÃ©lectionnÃ©e.';
      },
    });
  }
  ...
  private refreshOrgScopedLists(): void {
    ...
    this.data.getManagersByDepartement(dept).subscribe({
      next: (managers) => {
        ...
        // RH/Admin/Audit doivent pouvoir voir l'ensemble du tenant (pas seulement le dÃ©partement sÃ©lectionnÃ©).
        const std$ =
