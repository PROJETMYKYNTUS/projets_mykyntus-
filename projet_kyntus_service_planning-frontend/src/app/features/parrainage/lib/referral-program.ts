import type { ReferralProgramRules, ReferralBonusTier, SystemConfig } from '../models/system-config.model';
...
function clampTier(t: Partial<ReferralBonusTier>, fallback: ReferralBonusTier): ReferralBonusTier {
  return {
    ...
    afterMonths:
      Number.isFinite(t.afterMonths) && t.afterMonths! >= 0 ? Math.floor(t.afterMonths!) : fallback.afterMonths,
  };
}
...
export function getActiveTiersSorted(rules: ReferralProgramRules) {
