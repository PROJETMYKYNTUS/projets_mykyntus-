...
export class RequestDocumentPageComponent implements OnInit {
  ...
  /** ModÃ¨les actifs crÃ©Ã©s par les RH. */
  templates: DocumentTemplateListItemDto[] = [];
  docTypesLoading = false;
  docTypesError: string | null = null;

  /**
   * ClÃ© composite : `tpl:{uuid}` (modÃ¨le RH), `type:{uuid}` (type catalogue seul), ou `OTHER_KEY`.
   */
  selectionKey = '';
  ...
  /** LibellÃ© affichÃ© pour une variable (RH). */
  ...
  handleSubmit(ev: Event): void {
    ...
    if (this.selectionKey.startsWith(TPL_PREFIX)) {
      if (this.templateDetailLoading || !this.templateDetail) {
        this.submitError = 'Chargement du modÃ¨le en cours ou indisponible.';
        return;
      }
    ...
    if (this.selectionKey === OTHER_KEY) {
      ...
      if (!trimmed) {
        ...
        this.fieldValidationError = 'Veuillez dÃ©crire le document souhaitÃ© pour continuer.';
        return;
      }
      this.otherDescriptionError = false;
      const payload: CreateDocumentRequestPayload = {
  ...
  private formatHttpError(err: unknown): string {
    return formatDocumentationUxMessage(
      err,
      'Votre demande nâ€™a pas pu etre envoyee pour le moment. Merci de verifier les informations puis de reessayer.',
    );
  }
}
