    ...
    <div class="max-w-4xl mx-auto space-y-8">
      ...
      <section class="card-navy p-4 space-y-4">
        <div class="flex items-center gap-3 pb-3 border-b border-navy-800">
          <app-lucide-icon [icon]="cpuIcon" className="w-5 h-5 text-blue-500" />
          <h2 class="text-lg font-bold text-white">Rôle (démo)</h2>
        </div>
        <p class="text-xs text-slate-500">Simule un profil Parrainage pour tester le menu global.</p>
        <select
          [value]="role"
          (change)="onDemoRoleChange($event)"
...
export class GlobalSettingsPageComponent {
  ...
  onDemoRoleChange(event: Event): void {
    const code = (event.target as HTMLSelectElement).value as ParrainageRole;
    this.roleSvc.loginAsRole(code);
