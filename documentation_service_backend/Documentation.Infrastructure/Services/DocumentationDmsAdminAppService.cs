...
public sealed class DocumentationDmsAdminAppService(
    DocumentationDbContext db,
    ILogger<DocumentationDmsAdminAppService> logger) : IDocumentationDmsAdminAppService
{
    public async Task<AdminGeneralConfigDto?> GetGeneralConfigAsync(CancellationToken ct = default)
    {
        var row = await db.DmsGeneralConfigurations.AsNoTracking().OrderBy(x => x.Id).FirstOrDefaultAsync(ct);
        return row is null ? null : MapGeneral(row);
    }

    public async Task<AdminGeneralConfigDto?> SaveGeneralConfigAsync(AdminGeneralConfigDto body, CancellationToken ct = default)
    {
    ...
    public async Task<List<AdminDocTypeDto>> GetDocTypesAsync(CancellationToken ct = default)
    {
        var rows = await db.DocumentTypes.AsNoTracking().OrderBy(t => t.Code).ToListAsync(ct);
        return rows.Select(MapDocType).ToList();
    }

    public async Task<AdminDocTypeDto> CreateDocTypeAsync(CreateDocTypeRequestDto payload, CancellationToken ct = default)
    {
        if (!Guid.TryParse(payload.WorkflowId, out var wfId) || wfId == Guid.Empty)
            throw new DocumentationApiException(400, "workflowId invalide.");
        var wfExists = await db.Workflows.AnyAsync(w => w.Id == wfId, ct);
        if (!wfExists)
            throw new DocumentationApiException(400, "Workflow introuvable.");
        ...
        {
            throw new DocumentationApiException(409, "Code déjà utilisé ou contrainte refusée.");
        }

        logger.LogInformation("Admin CreateDocType id={Id} code={Code}", entity.Id, entity.Code);
        return MapDocType(entity);
    }

    public async Task<AdminDocTypeDto?> UpdateDocTypeAsync(Guid id, CreateDocTypeRequestDto payload, CancellationToken ct = default)
    {
        ...
        {
            throw new DocumentationApiException(409, "Contrainte refusée.");
        }
    ...
    public async Task<bool?> DeleteDocTypeAsync(Guid id, CancellationToken ct = default)
    {
        var entity = await db.DocumentTypes.FirstOrDefaultAsync(t => t.Id == id, ct);
        if (entity is null)
            return null;
        ...
        {
            throw new DocumentationApiException(409, "Impossible de supprimer : références existantes.");
        }
    ...
    public async Task<List<AdminWorkflowDefinitionDto>> GetWorkflowDefinitionsAsync(CancellationToken ct = default)
    {
    ...
    public async Task<AdminWorkflowDefinitionDto?> UpdateWorkflowDefinitionAsync(
        Guid id,
        AdminWorkflowDefinitionDto body,
        CancellationToken ct = default)
    {
        ...
        foreach (var stepDto in body.Steps)
        {
            if (!AppRoleHeaderParser.TryParse(stepDto.AssignedRole, out var role))
                throw new DocumentationApiException(400, $"Rôle invalide à l’étape {stepDto.Key}.");
            ...
            foreach (var a in stepDto.Actions)
            {
                if (!TryMapActionKey(a, out var ak))
                    throw new DocumentationApiException(400, $"Action invalide : {a}");
        ...
        return MapWorkflow(reloaded);
    }

    public async Task<List<AdminPermissionPolicyDto>> GetPermissionPoliciesAsync(CancellationToken ct = default)
    {
    ...
    public async Task<List<AdminPermissionPolicyDto>> SavePermissionPoliciesAsync(
        List<AdminPermissionPolicyDto> body,
        CancellationToken ct = default)
    {
        ...
        foreach (var p in body)
        {
            ...
            if (!AppRoleHeaderParser.TryParse(p.Role, out var role))
                throw new DocumentationApiException(400, $"Rôle invalide : {p.Role}");
        ...
        await db.SaveChangesAsync(ct);
        return await GetPermissionPoliciesAsync(ct);
    }

    public async Task<AdminStorageConfigDto?> GetStorageConfigAsync(CancellationToken ct = default)
    {
        var row = await db.DmsStorageConfigurations.AsNoTracking().OrderBy(x => x.Id).FirstOrDefaultAsync(ct);
        return row is null ? null : MapStorage(row);
    }

    public async Task<AdminStorageConfigDto?> SaveStorageConfigAsync(AdminStorageConfigDto body, CancellationToken ct = default)
    {
    ...
    public IReadOnlyList<string> GetAdminRoles() =>
        Enum.GetNames<AppRole>().Select(NormalizeRoleLabel).ToList();
