...
function mapLogToJournalRow(a: AuditLogDto): AuditRow {
  ...
  return {
    ...
    user: a.actorName ?? a.actorUserId ?? 'â€”',
    action: a.action,
    item: [a.entityType, a.entityId].filter(Boolean).join(' Â· ') || 'â€”',
    status: ok ? 'ValidÃ©' : 'En attente',
    departement: 'â€”',
    pole: 'â€”',
    cellule: 'â€”',
    roleMetier: 'â€”',
...
export class AuditJournalPageComponent implements OnInit, OnDestroy {
    ...
    this.sub.add(
      switchMapOnDocumentationContext(this.identity, () => this.api.getAllAuditLogs()).subscribe({
        next: (logs) => {
          ...
          this.anomalies = logs
            ...
            .map((a) => ({
              id: a.id,
              title: a.action,
              description: [a.entityType, a.entityId, a.errorMessage].filter(Boolean).join(' Â· ') || 'â€”',
              user: a.actorName ?? a.actorUserId ?? 'â€”',
              severity: 'CRITICAL',
            }));
          this.loading = false;
        ...
        error: () => {
          ...
          this.anomalies = [];
          this.loading = false;
          this.loadError = 'Impossible de charger les journaux dâ€™audit.';
        },
      }),
    );
  ...
  countRowsValide(): number {
    return this.rows.filter((r) => r.status === 'ValidÃ©').length;
