...
public static class DirectoryPrimeBootstrap
{
    ...
    private static async Task ImportAssignmentsAsync(HttpClient http, DirectoryDbContext db, CancellationToken ct)
    {
        ...
        foreach (var row in managers)
        {
            ...
            await UpsertAssignmentAsync(db, OrgAssignmentKind.ChefDeProjet, nodeId, OrgNodeLevel.Pole, employeeId, ct);
        }
        ...
        foreach (var row in supervisors)
        {
            ...
            if (nodeId.Length == 0) continue;
            await UpsertAssignmentAsync(db, OrgAssignmentKind.Superviseur, nodeId, OrgNodeLevel.Cellule, employeeId, ct);
        }
        ...
        foreach (var row in coaches)
        {
            ...
            if (nodeId.Length == 0) continue;
            await UpsertAssignmentAsync(db, OrgAssignmentKind.ReferentTechnique, nodeId, OrgNodeLevel.Service, employeeId, ct);
        }
    }
