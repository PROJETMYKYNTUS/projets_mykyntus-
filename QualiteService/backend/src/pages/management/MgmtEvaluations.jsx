...
function openEdit(scoreId) {
  window.dispatchEvent(
    new CustomEvent("kcq:navigate", {
      detail: { role: "management", view: "new", editScoreId: String(scoreId || "") },
    })
  );
}
...
export default function MgmtEvaluations() {
  ...
  const evaluatorOptions = useMemo(
    () => (evaluators || []).map((u) => ({ value: String(u.id || u._id), label: `${u.name || u.email || "—"}` })),
    [evaluators]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // This endpoint exists in many versions; if it doesn't, list stays empty.
        const res2 = await api.get("/users/evaluators");
        if (!mounted) return;
        setEvaluators(Array.isArray(res2.data) ? res2.data : []);
      } catch {
        // ignore
      }
    })();
  ...
  return (
    <div className="page">
      <PageHeader title="Évaluations" subtitle="Liste + filtres multi-sélection (optimisé performance)." />
