...
/** Métier : Pôle → Cellule → Service (pilote rattaché au service). teamId = legacy. */
...
export function organizationOfEmployee(e: Employee, departments: Department[]): Organization | null {
  ...
  // Phase 0 : utiliser poleId (nouvelle hiérarchie) si departementId absent.
  if (e.poleId && e.celluleId) {
    return { departementId: e.poleId, poleId: e.poleId, celluleId: e.celluleId };
  }
  return resolveOrganizationFromTeam(e.teamId ?? e.serviceId, departments);
}

export function resolveOrganizationFromTeam(teamId: string | undefined, departments: Department[]): Organization | null {
...
export function orgAllowedEmployeeIds(
  ...
  for (const e of employees) {
    ...
    if (!o) continue;
    if (viewerRole === 'Référent technique' || viewerRole === 'Coach') {
      if (viewer.serviceId) {
        if (e.serviceId === viewer.serviceId) ids.add(e.id);
      } else if (o.celluleId === viewerOrg.celluleId) {
        ids.add(e.id);
      }
      continue;
    }
    if (viewerRole === 'Superviseur' && o.celluleId === viewerOrg.celluleId) ids.add(e.id);
    if (viewerRole === 'Manager' && o.poleId === viewerOrg.poleId) ids.add(e.id);
    if ((viewerRole === 'Chef de projet' || viewerRole === 'RP') && o.departementId === viewerOrg.departementId) ids.add(e.id);
  }
  return ids;
}
