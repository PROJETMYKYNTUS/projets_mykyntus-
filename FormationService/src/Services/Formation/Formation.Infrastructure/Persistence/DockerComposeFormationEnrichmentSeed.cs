...
public static class DockerComposeFormationEnrichmentSeed
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Event-only : ne crée / ne modifie pas EmployeAnnuaire (employés intacts).
        logger?.LogInformation("Formation enrichment : annuaire non modifié (event-only).");
        // Parcours initiale en premier (priorité démo UI) — isolé du change tracker des autres étapes.
        await RunSeedStepAsync(db, "formation initiale", () => EnsureInitialTrainingPathsAsync(db, logger, ct), logger);
        await RunSeedStepAsync(db, "catalogue legacy", () => EnsureLegacyCatalogueAsync(db, logger, ct), logger);
        await RunSeedStepAsync(db, "training sessions", () => EnsureTrainingSessionsAsync(db, logger, ct), logger);
        await RunSeedStepAsync(db, "pilotage", () => EnsurePilotageAnnuaireAndSessionsAsync(db, logger, ct), logger);
        await RunSeedStepAsync(db, "sessions formateur", () => EnsureFormateurAnimatedSessionsAsync(db, logger, ct), logger);
    }

    private static async Task RunSeedStepAsync(
        FormationDbContext db,
        string name,
        Func<Task> step,
        ILogger? logger)
    {
        ...
        {
            db.ChangeTracker.Clear();
            logger?.LogWarning(ex, "Formation enrichment — étape « {Name} » ignorée.", name);
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        // Event-only : pas de création EmployeAnnuaire.
        logger?.LogInformation("Formation pilotage : annuaire non modifié (event-only).");
        ...
        await db.SaveChangesAsync(ct);
        logger?.LogInformation("Formation pilotage : {Count} sessions event-only.", sessions.Count);
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            await EnsureInitialTrainingDemoExtrasAsync(db, logger, ct);
            logger?.LogInformation("Formation initiale : parcours déjà seedés (extras vérifiés).");
            return;
        }
        ...
        // Workflow simulé : EnCours → quiz tracés → Valider formateur → file RH → prod / rejet.
        // Un parcours a DateFinPrevue à J+5 avec docs incomplets (alerte J-7).
        var paths = new List<InitialTrainingPath>
        {
            new()
            {
                Id = Guid.Parse("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa01"),
            ...
            new()
            {
                ...
                Status = InitialTrainingStatus.EnCours,
                FormateurComment = "Quiz en cours — scores saisis pour traçabilité",
            ...
            new()
            {
                ...
                FormateurComment = "Bon niveau — prêt à Valider / Prolonger / Rejeter",
                CreatedAt = now.AddDays(-20),
            ...
            new()
            {
                ...
                // J+5 → déclenche alerte docs manquants (fenêtre J-7)
                DateFinPrevue = now.AddDays(5),
                Status = InitialTrainingStatus.AttenteValidationRh,
                FormateurComment = "Validé formateur — en attente RH (docs partiels)",
            ...
            new()
            {
                ...
                DateFinPrevue = now.AddDays(7),
                Status = InitialTrainingStatus.AttenteValidationFormateur,
                FormateurComment = "Prêt validation formateur",
            ...
            new()
            {
                ...
                FormateurComment = "Passage production OK",
            ...
            new()
            {
                ...
                RejectReason = "Niveau insuffisant — reprise prévue",
        ...
        await db.SaveChangesAsync(ct);
        await EnsureInitialTrainingDemoExtrasAsync(db, logger, ct);
        logger?.LogInformation("Formation initiale : {Count} parcours créés (workflow démo).", paths.Count);
    }

    /// <summary>
    /// Quiz multi-résultats + checklist documents (partielle) pour tester UI formateur / RH / admin.
    /// Idempotent : ne recrée pas si déjà présents.
    /// </summary>
    private static async Task EnsureInitialTrainingDemoExtrasAsync(
        FormationDbContext db,
        ILogger? logger,
        CancellationToken ct)
    {
        ...
        var defs = await db.FormationDocumentDefinitions
            .Where(d => d.IsActive)
            .OrderBy(d => d.SortOrder)
            .ToListAsync(ct);
        if (defs.Count == 0)
        {
            logger?.LogWarning("Formation initiale extras : aucune définition document — checklist ignorée.");
        }

        var quizAdded = 0;
        var checklistTouched = 0;

        foreach (var path in paths)
        {
            if (path.QuizResults.Count == 0 && path.Status != InitialTrainingStatus.EnCours)
            {
                var samples = BuildDemoQuizResults(path, now);
                foreach (var q in samples)
                    db.InitialTrainingQuizResults.Add(q);
                quizAdded += samples.Count;

                // Agrégats legacy (affichage admin) alignés sur le dernier score
                var last = samples.OrderByDescending(s => s.RecordedAt).First();
                path.QuizScore = last.Score;
                path.QuizPassed = last.Passed;
                path.QuizRecordedBy = last.RecordedBy;
            }
            else if (path.QuizResults.Count == 0
                     && path.EmployeeName == InitialPathMarkerName)
            {
                // En cours : un seul quiz pour montrer la saisie formateur
                db.InitialTrainingQuizResults.Add(new InitialTrainingQuizResult
                {
                    InitialTrainingPathId = path.Id,
                    Title = "QCM Accueil & process",
                    Score = 68,
                    Passed = false,
                    RecordedBy = "Formateur démo",
                    RecordedAt = now.AddDays(-1),
                });
                path.QuizScore = 68;
                path.QuizPassed = false;
                path.QuizRecordedBy = "Formateur démo";
                quizAdded++;
            }

            if (defs.Count == 0) continue;

            var existingItems = await db.FormationDocumentChecklistItems
                .Where(i => i.InitialTrainingPathId == path.Id)
                .ToListAsync(ct);
            var byDef = existingItems.ToDictionary(i => i.DefinitionId);

            foreach (var def in defs)
            {
                if (!byDef.TryGetValue(def.Id, out var item))
                {
                    item = new FormationDocumentChecklistItem
                    {
                    ...
                    db.FormationDocumentChecklistItems.Add(item);
                    byDef[def.Id] = item;
                    checklistTouched++;
                }
            }

            // Scénarios documents (une seule fois si rien n'a encore été reçu)
            var ordered = defs.Select(d => byDef[d.Id]).ToList();
            if (ordered.Any(i => i.IsReceived))
                continue;

            if (path.Status == InitialTrainingStatus.EnProduction)
                MarkReceived(ordered, ordered.Count, now, "RH démo");
            else if (path.Status == InitialTrainingStatus.AttenteValidationRh)
                // Partiel : déclenche alerte J-7 (DateFinPrevue ≈ J+5)
                MarkReceived(ordered, Math.Max(1, ordered.Count / 2), now, "RH démo");
            else if (path.Status is InitialTrainingStatus.AttenteValidationFormateur
                     or InitialTrainingStatus.EnCours)
                MarkReceived(ordered, Math.Min(2, ordered.Count), now, "Formateur démo");
        }

        if (quizAdded > 0 || checklistTouched > 0)
        {
            await db.SaveChangesAsync(ct);
            logger?.LogInformation(
                "Formation initiale extras : {Quiz} quiz, {Checklist} items checklist.",
                quizAdded,
                checklistTouched);
        }
    }

    private static List<InitialTrainingQuizResult> BuildDemoQuizResults(InitialTrainingPath path, DateTime now)
    {
        return path.Status switch
        {
            ...
            [
                ...
                {
                    InitialTrainingPathId = path.Id,
                    Title = "QCM Process inbound",
                    Score = 78,
                    Passed = true,
                    RecordedBy = "Formateur démo",
                    RecordedAt = now.AddDays(-5),
                },
                new()
                {
                    InitialTrainingPathId = path.Id,
                    Title = "QCM Soft skills",
                    Score = 85,
                    Passed = true,
                    RecordedBy = "Formateur démo",
            ...
            InitialTrainingStatus.AttenteValidationRh =>
            [
                new()
                {
                    InitialTrainingPathId = path.Id,
            ...
            InitialTrainingStatus.EnProduction =>
            [
                new()
                {
                    InitialTrainingPathId = path.Id,
            ...
            [
                new()
                {
                    ...
                    Passed = false,
                    RecordedBy = "Formateur démo",
                    RecordedAt = now.AddDays(-40),
                },
                new()
                {
                    InitialTrainingPathId = path.Id,
                    Title = "QCM Process inbound (2e)",
                    Score = 55,
                    Passed = false,
                    RecordedBy = "Formateur démo",
                    RecordedAt = now.AddDays(-36),
                },
            ],
            _ =>
            [
    ...
    private static void MarkReceived(
        IReadOnlyList<FormationDocumentChecklistItem> items,
        int count,
        DateTime now,
        string by)
    {
        for (var i = 0; i < Math.Min(count, items.Count); i++)
        {
            ...
            item.ReceivedAt = now.AddDays(-i - 1);
            item.ReceivedBy = by;
        }
    }
