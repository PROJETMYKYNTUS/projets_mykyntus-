...
var directoryBase = builder.Configuration["Directory:BaseUrl"] ?? "http://employee-directory-backend:8080";

builder.Services.AddScoped<PrimeOrgStructureCommandService>();
builder.Services.AddScoped<PrimeAdminDbReadService>();
builder.Services.AddKyntusOutbox<PrimeDbContext>();
builder.Services.AddKyntusIam();
builder.Services.AddScoped<IPermissionCatalog, PrimePermissionCatalog>();
builder.Services.AddHttpClient<IRebacClient, DirectoryHttpRebacClient>(c => c.BaseAddress = new Uri(directoryBase));
builder.Services.AddHttpClient<IPrimeDirectoryAsOfClient, PrimeDirectoryAsOfClient>(c => c.BaseAddress = new Uri(directoryBase));
