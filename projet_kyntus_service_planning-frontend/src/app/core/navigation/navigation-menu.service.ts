...
export class NavigationMenuService {
  ...
  private filterPrimeChildren(): MenuItem[] {
    ...
    return buildPrimeMenuItemsForRole(role).filter((item) => this.isPrimeItemAllowed(item, role));
  }
  ...
  private isPrimeItemAllowed(item: MenuItem, role: Role): boolean {
    ...
    if (!isPrimePathAllowedForRole(path, role)) return false;
    return this.primePermissions.canViewPath(role, path);
  }
