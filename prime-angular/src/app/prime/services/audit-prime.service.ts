...
export const AuditPrimeService = {
  getDashboard: async () => {
    ...
    const validated = summary.rhApproved;
    const rejected = summary.rejected;
    return {
      kpis: {
        totalPrimes: summary.total,
        validations: summary.total - summary.pending,
        anomalies: openAnomalies,
        conformityRate: summary.total === 0 ? 100 : Math.round((100 * validated) / Math.max(validated + rejected, 1)),
      },
      charts: {
        flowByStep: [
          { step: 'Réf. tech.', value: summary.referentTechniqueApproved },
          { step: 'Superviseur', value: summary.superviseurApproved },
          { step: 'CdP', value: summary.chefDeProjetApproved },
          { step: 'RH', value: summary.rhApproved },
        ],
        ...
        activityByRole: [
          ...
          { role: 'En attente', value: summary.pending },
  ...
  getAnomalies: async (): Promise<AuditAnomaly[]> => {
    ...
    return rows.map((r) => ({
      ...
      status: r.status === 'Resolved' ? 'Corrigée' : 'Ouverte',
    }));
  },
};
