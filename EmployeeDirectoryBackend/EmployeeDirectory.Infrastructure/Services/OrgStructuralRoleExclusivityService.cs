    ...
    IOutboxWriter outbox) : IOrgStructuralRoleExclusivityService
{
    ...
    private static string RoleLabel(OrgAssignmentKind kind) => kind switch
    {
        OrgAssignmentKind.ChefDeProjet => "Chef de projet",
        OrgAssignmentKind.Superviseur => "Superviseur",
        OrgAssignmentKind.ReferentTechnique => "Référent technique",
        OrgAssignmentKind.Pilote => "Pilote",
        _ => kind.ToString(),
    };
