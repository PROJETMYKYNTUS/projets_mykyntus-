...
@Component({
  ...
  template: `
    <div class="space-y-4">
      <div class="rounded-lg border border-emerald-500/25 bg-emerald-500/5 px-4 py-3 text-xs text-slate-300">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
          ...
          <input [value]="q()" (input)="q.set($any($event.target).value)" placeholder="Rechercher utilisateur, IP, lieu…"
            class="w-full bg-navy-900 border border-navy-800 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-200 transition-colors focus:border-emerald-500/40 focus:ring-1 focus:ring-emerald-500/20" />
        </div>
        @if (hasBrute()) {
      ...
      <div class="card-navy overflow-x-auto border border-navy-800/80">
        <table class="w-full text-sm min-w-[920px]">
          <thead class="bg-navy-800/55 text-slate-400 text-left">
            <tr>
          ...
          <tbody class="divide-y divide-navy-800">
            @for (r of filtered(); track r.id) {
              <tr class="hover:bg-navy-800/35 transition-colors">
                <td class="px-4 py-3 text-slate-200 font-medium">{{ r.user }}</td>
                <td class="px-4 py-3 text-slate-400 whitespace-nowrap">{{ r.datetime }}</td>
                <td class="px-4 py-3 text-slate-300 font-mono text-xs">{{ r.ip }}</td>
                <td class="px-4 py-3 text-slate-400">{{ r.location }}</td>
                <td class="px-4 py-3">
                  @if (r.success) {
                ...
                <td class="px-4 py-3 text-slate-300">{{ typeLabel(r) }}</td>
                <td class="px-4 py-3 text-slate-500 text-xs max-w-[240px]">{{ r.detail ?? '—' }}</td>
              </tr>
            }
          </tbody>
        </table>
