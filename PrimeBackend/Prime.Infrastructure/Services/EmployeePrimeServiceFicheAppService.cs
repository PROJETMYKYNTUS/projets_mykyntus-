...
public sealed class EmployeePrimeServiceFicheAppService(
    PrimeDbContext db,
    ...
    AnomalyDetectionService anomalies) : IEmployeePrimeServiceFicheAppService
{
    ...
    public async Task<IReadOnlyList<EmployeePrimeServiceFicheListItemDto>> ListAsync(
        string? serviceId,
        string? celluleId,
        string period,
        ...
        CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(period) || string.IsNullOrWhiteSpace(supervisorUserId))
            throw new ArgumentException("period et supervisorUserId sont requis.");
        ...
        if (!hasService && !hasCellule)
            throw new ArgumentException("Indiquez serviceId (équipe) ou celluleId (cellule entière).");
    ...
    public async Task<EmployeePrimeServiceFicheResponseDto> GetForEmployeeAsync(
        string supervisorUserId,
        string employeeId,
        string period,
        string? templateId,
        CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(supervisorUserId) || string.IsNullOrWhiteSpace(employeeId) ||
            ...
            throw new ArgumentException("supervisorUserId, employeeId et period sont requis.");
        ...
        if (draft is null)
        {
            throw new KeyNotFoundException(string.IsNullOrWhiteSpace(templateId)
        ...
        if (fiche is null)
        {
            return new EmployeePrimeServiceFicheResponseDto
            {
                Id = Guid.Empty,
        ...
        return Map(fiche, draft);
    }

    public async Task<EmployeePrimeServiceFicheResponseDto> UpsertAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        var draft = await db.SupervisorCellulePrimeDrafts.FirstOrDefaultAsync(x => x.Id == body.CellulePrimeDraftId, ct)
            ?? throw new KeyNotFoundException("Brouillon pôle introuvable.");
        if (!string.Equals(draft.SupervisorUserId, body.SupervisorUserId.Trim(), StringComparison.Ordinal) ||
            ...
            throw new ArgumentException("Le brouillon pôle ne correspond pas à l'employé ou à la période.");
        ...
        if (PrimeEmployeeFicheAmountService.HasNegativeFinancialValuesInServiceSaisieJson(body.ServiceSaisieJson))
            throw new ArgumentException(DbExceptionMessages.NonNegativePrimeAmountsRequired);
        ...
        if (entity is null)
        {
            entity = new EmployeePrimeServiceFiche
            {
                Id = Guid.NewGuid(),
                CellulePrimeDraftId = draft.Id,
        ...
        return Map(entity, draft);
    }

    public async Task<EmployeePrimeServiceFicheResponseDto> PersistAmountsAsync(
        Guid ficheId,
        ...
        CancellationToken ct = default)
    {
        ...
        var entity = await db.EmployeePrimeServiceFiches.FirstOrDefaultAsync(x => x.Id == ficheId, ct)
            ?? throw new KeyNotFoundException("Fiche introuvable.");
