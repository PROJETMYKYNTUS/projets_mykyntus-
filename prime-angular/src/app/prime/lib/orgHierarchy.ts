...
/**
 * Hiérarchie stricte : Pilote → Coach → Superviseur → Manager → RP (RP = sommet).
...
export function isTargetUnderViewerInChain(
  ...
  let cur: Employee | undefined = employees.find((e) => e.id === targetEmployeeId);
  ...
  while (cur && cur.parentId) {
    ...
    cur = employees.find((e) => e.id === parentId);
  }
  return false;
}
...
export function employeeVisibleToHierarchyRole(
  ...
  if (viewerRole === 'Manager' || viewerRole === 'Superviseur' || viewerRole === 'Coach') {
