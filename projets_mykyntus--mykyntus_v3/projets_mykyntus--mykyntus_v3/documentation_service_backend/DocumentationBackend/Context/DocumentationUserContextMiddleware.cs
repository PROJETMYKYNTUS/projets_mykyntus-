...
public sealed class DocumentationUserContextMiddleware(RequestDelegate next)
{
        ...
        ILogger<DocumentationUserContextMiddleware> logger)
    {
        ...
        var tenantHeader = httpContext.Request.Headers[DocumentationInboundHeaders.TenantId].ToString();
        var userHeader = httpContext.Request.Headers[DocumentationInboundHeaders.UserId].ToString();
        logger.LogWarning(
            "[doc-headers-debug] X-Tenant-Id brut={TenantHeader} résolu contexte.TenantId={TenantResolved} | X-User-Id brut={UserHeader} résolu contexte.UserId={UserResolved}",
            string.IsNullOrEmpty(tenantHeader) ? "(absent)" : tenantHeader,
            userContext.TenantId ?? "(null)",
            string.IsNullOrEmpty(userHeader) ? "(absent)" : userHeader,
