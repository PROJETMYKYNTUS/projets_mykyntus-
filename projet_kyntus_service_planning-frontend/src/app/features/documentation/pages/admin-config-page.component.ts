...
export class AdminConfigPageComponent implements OnInit {
  ...
  validate(): Record<string, string> {
    ...
    if (!this.draft.systemName.trim()) nextErrors['systemName'] = 'Nom du syst̬me requis.';
    if (Number.isNaN(this.draft.maxFileSizeMB) || this.draft.maxFileSizeMB <= 0) {
      nextErrors['maxFileSizeMB'] = 'Taille maximale doit ̻tre > 0.';
    }
    if (this.draft.retentionDays < 0) nextErrors['retentionDays'] = "Dur̩e d'archivage ne peut pas ̻tre n̩gative.";

    if (this.derivedAllowedTypes().length === 0)
      nextErrors['allowedFileTypes'] = 'D̩finissez au moins un type de fichier (ex: pdf, docx).';

    if (this.draft.autoNumberingEnabled && !this.draft.numberingPattern.trim()) {
      nextErrors['numberingPattern'] = 'Motif de num̩rotation requis si la num̩rotation est activ̩e.';
    }
  ...
  async onSave(): Promise<void> {
    ...
    try {
      ...
      this.draft = next;
      this.successMessage = 'Configuration sauvegard̩e.';
    } finally {
      this.saving = false;
    }
  }
