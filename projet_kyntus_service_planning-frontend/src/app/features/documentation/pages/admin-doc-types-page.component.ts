...
export class AdminDocTypesPageComponent implements OnInit {
  ...
  validateForm(): Record<string, string> {
    ...
    if (!this.form.department.trim()) e['department'] = 'DÃ©partement requis.';
    if (!this.form.workflowId.trim()) e['workflowId'] = 'Workflow requis.';
    if (Number.isNaN(this.form.retentionDays) || this.form.retentionDays < 0) e['retentionDays'] = 'DurÃ©e invalide.';
    return e;
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      await this.reload();
      this.modalOpen = false;
      this.successMessage = wasCreate ? 'Type de document ajoutÃ©.' : 'Type de document mis Ã  jour.';
  ...
  async onDelete(id: string): Promise<void> {
    ...
    try {
      ...
      await this.reload();
      this.successMessage = 'Type de document supprimÃ©.';
      this.notify.showSuccess('Le type de document a ete supprime.');
    } catch (error) {
      this.notify.showError(
        formatDocumentationUxMessage(
          error,
          'La suppression nâ€™a pas pu etre finalisee pour le moment. Merci de reessayer.',
        ),
      );
    } finally {
