...
const PRIME_DIR = path.join(ROOT, 'src/app/features/prime');
...
for (const file of walk(PRIME_DIR)) {
  ...
  if (content !== original) {
    ...
    changed.push(path.relative(PRIME_DIR, file).replace(/\\/g, '/'));
  }

  const tw = (content.match(/text-white/g) || []).length;
  if (tw > 0) {
    report.textWhite[path.relative(PRIME_DIR, file).replace(/\\/g, '/')] = tw;
  }
}
...
fs.writeFileSync(path.join(ROOT, 'MIGRATION_AGENT_REPORT.txt'), JSON.stringify(report, null, 2));
