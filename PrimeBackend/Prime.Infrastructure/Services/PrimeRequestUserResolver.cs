    ...
    PrimeJwtEmployeeProvisioner jwtProvisioner) : IPrimeRequestUserResolver
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (request is null)
            return await TryResolveForValidationCoreAsync(queryUserId, queryRole, ct);
        return await TryResolveForValidationAsync(request, queryUserId, queryRole, ct);
    }

    public async Task<PrimeResolvedUser?> TryResolveAsync(string? bodyUserId, string? bodyRole, CancellationToken ct = default)
    {
        var request = httpContextAccessor.HttpContext?.Request;
        if (request is null)
            return null;
        return await TryResolveAsync(request, bodyUserId, bodyRole, ct);
    }
    ...
    public async Task<PrimeResolvedUser?> TryResolveForValidationAsync(
        ...
        string? queryRole,
        CancellationToken ct = default) =>
        await TryResolveForValidationCoreAsync(queryUserId, queryRole, ct, request);

    private async Task<PrimeResolvedUser?> TryResolveForValidationCoreAsync(
        string? queryUserId,
        string? queryRole,
    ...
    {
        var headerUserId = request?.Headers[IPrimeRequestUserResolver.HeaderUserId].FirstOrDefault();
        var headerRole = request?.Headers[IPrimeRequestUserResolver.HeaderRole].FirstOrDefault();
        var userId = FirstNonEmpty(headerUserId, bodyUserId);
        var roleRaw = FirstNonEmpty(headerRole, bodyRole);
        ...
        return new PrimeResolvedUser(emp.Id, role, MapEmployee(emp));
    }

    private static Employee MapEmployee(EmployeeEntity e) => new()
    {
        Id = e.Id,
        ...
        CancellationToken ct)
    {
        ...
        return new PrimeResolvedUser(emp.Id, IPrimeRequestUserResolver.ExpandRole(role), MapEmployee(emp));
    }
