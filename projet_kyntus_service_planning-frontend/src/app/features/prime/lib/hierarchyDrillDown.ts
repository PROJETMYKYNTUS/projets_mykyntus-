...
/** Sélection cascade (Manager/Superviseur : coachId ; RP : managerId + coachId). */
...
export function listCoachesUnderManager(employees: Employee[], managerId: string): Employee[] {
  const superviseurIds = listChildrenByRole(employees, managerId, 'Superviseur').map((e) => e.id);
  const coachesViaSuperviseur = superviseurIds.flatMap((id) => listChildrenByRole(employees, id, 'Coach'));
  const directCoaches = listChildrenByRole(employees, managerId, 'Coach');
  return [...directCoaches, ...coachesViaSuperviseur];
}
...
export function piloteIdsForManagerDrill(
  employees: Employee[],
  managerOrSuperviseurId: string,
  coachId: string | undefined,
): Set<string> | null {
  if (!coachId) return null;
  const coach = employees.find((e) => e.id === coachId);
  if (!coach || coach.role !== 'Coach') return new Set();
  const directLink = coach.parentId === managerOrSuperviseurId;
  const viaSuperviseur = employees.some(
    (e) => e.id === coach.parentId && e.role === 'Superviseur' && e.parentId === managerOrSuperviseurId,
  );
  if (!directLink && !viaSuperviseur) return new Set();
...
export function piloteIdsForRpDrill(
  ...
  if (!coach || coach.role !== 'Coach') return new Set();
  const directLink = coach.parentId === drill.managerId;
  const viaSuperviseur = employees.some(
    (e) => e.id === coach.parentId && e.role === 'Superviseur' && e.parentId === drill.managerId,
  );
  if (!directLink && !viaSuperviseur) return new Set();
...
/**
 * Filtre strict drill-down : aucun accès Pilote sans Coach (Manager/Superviseur) ;
...
export function drillSelectOptions(
  ...
  const coachesForManager =
    viewerRole === 'Manager' || viewerRole === 'Superviseur'
