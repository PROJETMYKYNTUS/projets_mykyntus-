...
export class DocumentationShellComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.forcedRoleFromQuery) {
      // Le shell Planning fournit ?doc=rh|pilot : ce paramètre doit primer sur tout contexte local précédent.
    ...
    if (devTools) {
      // En mode dev local, on évite d'écraser un mode explicite transmis par Planning (?doc=rh|pilot).
      if (!this.forcedRoleFromQuery) {
  ...
  /**
   * Evite la fuite de contexte entre sessions (ex: login Fatima après Yasmine).
   * Si le token courant appartient à un autre email que le cache local Documentation, on purge les clés.
   */
