...
public sealed class PrimeFicheValidationSubmissionService(PrimeDbContext db, PrimeValidationWorkflowRuntime wfRuntime)
{
    ...
    /// <summary>Résout le brouillon pôle validé pour une fiche (id lié ou couple superviseur/cellule/période).</summary>
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var linked = await db.SupervisorCellulePrimeDrafts
                .FirstOrDefaultAsync(d => d.Id == fiche.CellulePrimeDraftId, ct);
            if (linked is not null && IsDraftValidated(linked.Status))
                return linked;
        }

        return await db.SupervisorCellulePrimeDrafts
            .Where(d =>
                d.SupervisorUserId == fiche.SupervisorUserId &&
                d.CelluleId == fiche.CelluleId &&
