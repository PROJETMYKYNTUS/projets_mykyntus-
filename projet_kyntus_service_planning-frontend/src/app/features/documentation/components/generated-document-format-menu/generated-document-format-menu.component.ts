...
export class GeneratedDocumentFormatMenuComponent implements OnDestroy {
  ...
  /** Document gÃ©nÃ©rÃ© cible (export multi-format). */
  @Input() generatedDocumentId: string | null = null;
  /** Base de nom de fichier sans extension (si lâ€™API nâ€™envoie pas Content-Disposition). */
  @Input() fileNameBase: string | null = null;
  /**
   * <c>button</c> / <c>standard</c> : libellÃ© Â« â¬‡ TÃ©lÃ©charger Â» (<c>.btn-download</c>).
   * <c>icon</c> : bouton compact mÃªme style (liste Ã©troite).
   */
  @Input() appearance: 'button' | 'icon' | 'standard' = 'button';
  menuOpen = false;
  busyFormat: GeneratedExportFormat | null = null;
  /** Position du panneau (fixed) pour Ã©viter la coupure par overflow des tableaux / cartes. */
  dropdownTop = 0;
  dropdownLeft = 0;
      ...
      this.data.exportGeneratedDocument(id, format).subscribe({
        ...
        error: (e: unknown) => {
          ...
          if (format === 'pdf' && e instanceof HttpErrorResponse && e.status === 422 && id) {
            this.notify.showSuccess('Le PDF gÃ©nÃ©rique nâ€™est pas disponible : lancement du tÃ©lÃ©chargement Word (mÃªme mise en page que le modÃ¨le).');
