...
public sealed class AnomalyDetectionService(PrimeDbContext db, PrimeValidationWorkflowRuntime wfRuntime)
{
    ...
    private static Anomaly BuildAnomaly(EmployeePrimeServiceFiche f, DateTimeOffset now, string type, string severity, string description, string? contextJson) =>
        new()
        {
            Id = Guid.NewGuid(),
            DetectedAt = now,
            ...
            TargetEntityType = nameof(EmployeePrimeServiceFiche),
    ...
    private async Task<int> UpsertAnomaliesAsync(List<Anomaly> detected, DateTimeOffset now, CancellationToken ct, string? scopedTargetId = null)
    {
