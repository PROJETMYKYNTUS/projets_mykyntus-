...
namespace AuthService.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
            ...
            [FromBody] RegisterFromPlanningDto dto)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(dto.RoleName))
                    role = await _roleRepository.GetByNameAsync(dto.RoleName.Trim());
                if (role is null && dto.RoleId > 0)
                    role = await _roleRepository.GetByIdAsync(dto.RoleId);
                if (role == null)
                {
        ...
        public async Task<IActionResult> DeleteFromPlanning(int authUserId)
        {
            try
            {
                var deleted = await _userRepository.DeleteAsync(authUserId);
                if (!deleted)
