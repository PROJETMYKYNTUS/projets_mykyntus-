    ...
    @if (loading) {
      ...
      <div class="card-navy p-12 text-center">
        <div class="w-16 h-16 bg-card rounded-full flex items-center justify-center mx-auto mb-4">
          <app-lucide-icon [icon]="usersIcon" className="w-8 h-8 text-muted" />
        </div>
        <h4 class="text-primary font-medium">Aucun membre</h4>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-card/50 border-b border-default">
                <th class="px-6 py-4 text-[11px] font-bold text-muted uppercase tracking-wider">Nom</th>
                <th class="px-6 py-4 text-[11px] font-bold text-muted uppercase tracking-wider">Rôle</th>
                <th class="px-6 py-4 text-[11px] font-bold text-muted uppercase tracking-wider">Projet</th>
                <th class="px-6 py-4 text-[11px] font-bold text-muted uppercase tracking-wider">Parrainages</th>
                <th class="px-6 py-4 text-[11px] font-bold text-muted uppercase tracking-wider">Taux de succès</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-default">
              @for (m of members; track m.id) {
                <tr class="hover:bg-input/30 transition-colors">
                  <td class="px-6 py-4 text-sm font-medium text-primary">{{ m.name }}</td>
                  <td class="px-6 py-4 text-sm text-muted">{{ m.role }}</td>
