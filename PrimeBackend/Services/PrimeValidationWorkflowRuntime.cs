...
/// <summary>
/// Workflow fiches : transitions issues uniquement de <see cref="WorkflowStepConfigEntity"/> (aucun ordre codé en dur).
/// </summary>
public sealed class PrimeValidationWorkflowRuntime(PrimeDbContext db)
{
    public async Task<WorkflowStepConfigEntity?> FindActiveStepAsync(string fromStatus, string approverRole, CancellationToken ct)
    {
    ...
    public async Task<(bool ok, string? error, string? nextStatus, WorkflowStepConfigEntity? step)> TryResolveApprovalAsync(
