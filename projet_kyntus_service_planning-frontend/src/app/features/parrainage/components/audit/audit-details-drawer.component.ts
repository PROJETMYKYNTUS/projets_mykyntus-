      ...
      <div class="fixed inset-0 z-50 flex justify-end bg-app/55">
        ...
        <div class="w-full max-w-md h-full bg-input border-l border-default p-5 space-y-4 overflow-y-auto shadow-2xl">
          <div class="flex items-center justify-between">
            <h4 class="text-lg font-semibold text-primary">Détail technique</h4>
          ...
          <div class="text-sm space-y-3">
            ...
            <div>
              ...
              <div class="mt-1 grid grid-cols-1 gap-2 sm:grid-cols-2">
                ...
                <pre class="p-3 rounded-lg bg-app border border-[var(--success-border)] text-[11px] text-[var(--success-text)] overflow-x-auto max-h-48">{{ json(selected.afterState) }}</pre>
              </div>
            </div>
            <div>
            ...
            <div class="flex flex-col gap-2 pt-2">
              <button type="button" (click)="openUserTimeline.emit()" class="w-full py-2.5 rounded-lg border border-[var(--info-border)] bg-[var(--info-bg)] text-[var(--info-text)] text-sm hover:border-[var(--soft-blue)] transition-colors">
                Voir toutes les actions de cet utilisateur (timeline)
              </button>
              <button type="button" (click)="investigateUser.emit()" class="w-full py-2.5 rounded-lg border border-[var(--warning-border)] bg-[var(--warning-bg)] text-[var(--warning-text)] text-sm hover:border-[var(--warning)] transition-colors">
