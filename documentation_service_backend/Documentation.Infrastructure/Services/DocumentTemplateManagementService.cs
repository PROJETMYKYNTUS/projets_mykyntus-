...
using Documentation.Infrastructure.DocxParsing;
    ...
    ILogger<DocumentTemplateManagementService> logger) : IDocumentTemplateManagementService
{
        ...
        CancellationToken ct)
    {
        ...
        var version = await CreateTemplateVersionInternalAsync(
            ...
            "published",
            originalAssetUri,
            kind == DocumentTemplateKind.Static ? Array.Empty<TemplateVariableInput>() : variables,
            userId,
        ...
        CancellationToken ct)
    {
        ...
        var rows = variables
            ...
            .Select((v, index) => new DocumentTemplateVariable
            {
                ...
                FormScope = string.IsNullOrWhiteSpace(v.FormScope) ? "hr" : v.FormScope.Trim().ToLowerInvariant(),
        ...
        if (template.CurrentVersionId.HasValue)
        {
            var vars = await db.DocumentTemplateVariables.AsNoTracking()
                ...
                .Select(v => new DocumentTemplateVariableResponse(
                    ...
                    v.ValidationRule,
                    v.DisplayLabel,
                    string.IsNullOrWhiteSpace(v.FormScope) ? "hr" : v.FormScope,
                    v.SourcePriority,
                    v.NormalizedName,
                    v.RawPlaceholder,
                    v.SortOrder))
